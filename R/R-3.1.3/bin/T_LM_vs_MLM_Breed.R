...
for(breed in breeds)
{
	...
	plot(LWdataB$dim, LWdataB$milk, type='p', xlab='DIM', ylab='Milk', main=paste("LM=blue, MLM=red, Overrall=green, Breed=", breed, sep=''))
	...
	lines(bin$dim, bin$milk_lm) 

    par(pch=22, col="red") # plotting symbol and color 
	lines(bin$dim, bin$milk_mlm) 

	par(pch=22, col="green") # plotting symbol and color 
	lines(bin$dim, bin$milk) 
...
plot(chisq_comparison$Breed, chisq_comparison$lmPValue, pch=22, col='blue', main='ChiSq p-value: LM=blue.dash MLM=red.line')
lines(chisq_comparison$Breed, chisq_comparison$mlmPValue, col='red')
